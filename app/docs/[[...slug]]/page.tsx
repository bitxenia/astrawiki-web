...
export default function Pages({ params: { slug = [] } }: PageProps) {
    const [parsedMarkdown, setParsedMarkdown] = useState<ReactElement<any, any> | null>(null);
    ...
    return (
        <div className="flex items-start gap-14">
            <div className="flex-[3] pt-10">
                ...
                {parsedMarkdown && <Typography>
                    <h1 className="text-3xl -mt-2">{pathName}</h1>
                    <div>{parsedMarkdown}</div>
                    <Pagination pathname={pathName} />
                </Typography>}
                {!parsedMarkdown && <div className="flex justify-center items-center min-h-screen">
                    <BarLoader />
                </div>}
            </div>
